...
//   // === Filtering Logic (same as Dashboard) ===
...
//               <div
//                 className={`w-8 h-8 bg-${card.color}-100 rounded-lg flex items-center justify-center`}
//               >
...
export default function JobListPage() {
  ...
  return (
    <div className="">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-6">
        {[
          { label: "Total Jobs", value: jobs.length, color: "blue", emoji: "📋" },
          { label: "Assigned", value: jobs.length, color: "yellow", emoji: "👤" },
          { label: "Pending", value: pendingCount, color: "purple", emoji: "⏱" },
          { label: "Completed", value: completedCount, color: "blue", emoji: "✓" },
        ].map((card) => (
          <div
            key={card.label}
            className="bg-white rounded-lg p-4 shadow-sm transition hover:shadow-md"
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <div
                className={`w-8 h-8 bg-${card.color}-100 rounded-lg flex items-center justify-center`}
              >
                <span className={`text-${card.color}-600 text-lg`}>{card.emoji}</span>
              </div>
            </div>
            <p className="text-2xl font-bold text-gray-800">
              {card.value.toString().padStart(2, "0")}
          ...
          <div className="flex w-full lg:w-auto gap-4">
            <div className="relative flex-1 lg:flex-none" ref={statusDropdownRef}>
              <button
                ...
                className="flex items-center justify-between gap-3 px-4 py-2 border border-gray-300 rounded-lg text-sm bg-white hover:border-gray-400 transition-colors cursor-pointer w-full lg:min-w-[150px]"
              >
